...
│   ├── SpaceView.vue           ← Space page: visualization tab bar + active visualization
...
| Add a field to Space or Visualization | `src/types/spaces.ts` or `src/types/visualizations.ts`, then propagate similarly |
...
| Add a space action | `src/stores/spaces/mutations.ts` or `space-membership.ts` |
